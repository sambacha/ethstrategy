...
import {TimelockController, ERC1155Holder, ERC721Holder} from "@openzeppelin/contracts/governance/TimelockController.sol";
...
import {Timelock} from "solady/src/accounts/Timelock.sol";
    ...
    GovernorTimelockControl
{
    ...
    function _tallyUpdated(
        uint256 proposalId
    ) internal override(GovernorPreventLateQuorum, Governor) {
    ...
    function proposalNeedsQueuing(
        uint256 proposalId
    ) public view override(GovernorTimelockControl, Governor) returns (bool) {
    ...
    function onERC1155Received(
        ...
        uint256,
        bytes memory
    ) public virtual override(ERC1155Holder, Governor) returns (bytes4) {
    ...
    function onERC721Received(
        ...
        uint256,
        bytes memory
    ) public pure override(ERC721Holder, Governor) returns (bytes4) {
        ...
        returns (bool)
    {
        return
            ...
            TimelockController.supportsInterface(interfaceId);
    }
    ...
    function CLOCK_MODE()
        ...
        view
        virtual
        override(Governor, GovernorVotes, Votes)
        returns (string memory)
    {
    ...
    function clock()
        ...
        view
        virtual
        override(Votes, Governor, GovernorVotes)
        returns (uint48 result)
    {
        return uint48(block.timestamp);
    }

    function addNavToken(
        address _navToken
    ) public onlyRole(ADMIN_ROLE) {
        navTokens.push(_navToken);
    }

    function removeNavToken(
        address _navToken
    ) public onlyRole(ADMIN_ROLE) {
        uint256 i = 0;
    ...
    function getNavValue(
        uint256 _amount,
        uint256 _discount
    ) external view returns (NavValue[] memory navValues) {
        return _getNavValue(_amount, _discount);
    }

    function _getNavValue(
        uint256 _amount,
        uint256 _discount
    ) internal view returns (NavValue[] memory navValues) {
        if (_amount == 0) {
            revert AmountIsZero();
        }
    ...
    function setRageQuitPenalty(uint16 _rageQuitPenalty) public onlyRole(ADMIN_ROLE) {
